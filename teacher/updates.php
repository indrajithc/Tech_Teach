...
//echo $_SERVER["SERVER_NAME"];
	...
	$imgFpath4553535 = $_SERVER['DOCUMENT_ROOT'] .'tech_teach/teacher/images_/'.$result_view_base['image'];
								
						
								if(file_exists($imgFpath4553535) && strlen($result_view_base['image'])>0 ) {
									
									$imgFpath4553535 = PATH.'/teacher/images_/'.$result_view_base['image'];
								} else {
									$imgFpath4553535 = "../assets/images/defalut.jpg";	
								}
...
<div id="default_name_id_save_deta" class="hidden" myname = "<?php echo $myname; ?>"  myimg="<?php echo $imgFpath4553535; ?>"  user_name="<?php echo substr($_SESSION['te'],3); ?>"></div>
 ...
 <?php
					...
					foreach($result_view_post as $value) {
						...
						if (!ctype_digit($value['te_id'])) { 
								...
								$INRquery = "SELECT d.name as department ,t.image, CONCAT( t.fname,' ',t.lname ) AS name, t.delete_status FROM teacher t LEFT JOIN department d on t.department = d.did WHERE t.user_name ='".$value['te_id']."'";
								$INRresult_post = $a->display($INRquery);
								if(!empty($INRresult_post)) {
					...
					//	 var_dump($INRquery);	
								...
								$INRquery = "SELECT class AS department ,image, CONCAT( fname,' ',lname ) AS name  , delete_status FROM `student` WHERE user_name ='".$value['te_id']."'";
								$INRresult_post = $a->display($INRquery);
								$errors = array_filter($INRresult_post);
					// var_dump($INRquery);	
					 $boolTestT = true;
...
<i class="fa fa-pencil-square-o oclasForIvondWithClcik" aria-hidden="true"  post_id="<?php echo $value['post_id']?>"><span style="padding-left:13px">edit</i>
 ...
 <?php 
				...
				if(file_exists($imgFpath54345345) && strlen($value['attachment'])>0 ) {
                    ...
                    <div class="post-materials" style="width: 100%;">
                        <div class="" style="text-align:center ;  max-width: 600px;">
                            <img src="<?php  echo ''.PATH.'/post/'.$value['attachment']; ?>" style="max-width:100%; max-height:500px; width:auto; height:auto;">
                        </div>
                    </div>
                    <?php
						break;
  ...
  <source src="<?php  echo ''.PATH.'/post/'.$value['attachment']; ?>" type="video/<?php echo pathinfo($value['attachment'], PATHINFO_EXTENSION); ?>">
Your browser does not support the video tag.
                    ...
                    <div class="post-materials" style="width: 100%;">
                        <div class="" style="text-align:center ;  max-width: 600px;">
                            <object style=" width:100%;height:500px;border: none;" src="<?php echo ''.PATH.'/post/'.$value['attachment'];?>"><embed  style=" width:100%;height:500px;border: none;"src="<?php echo ''.PATH.'/post/'.$value['attachment'];?>"></embed></object>
                    ...
                    <div class="post-materials" style="width: 100%;">
                        <div class="" style="text-align:center ;  max-width: 600px;">
                            <embed src="<?php  echo ''.PATH.'/post/'.$value['attachment']; ?>" style=" width:100%; height:500px;">
                        </div>
                    </div>
                    <?php
						break;
        ...
        <div class="post-comment-wrapper">
            <!-- Comments -->
             <?php 
							 ...
							 $INRquery = "SELECT * FROM `teacher` WHERE  user_name ='".$valueForComm['user_id']."'";
					...
					$imgFpath = $_SERVER['DOCUMENT_ROOT'] .'tech_teach/teacher/images_/'.$INRresult_Comment['image'];
								...
								if(file_exists($imgFpath) && strlen($INRresult_post['image'])>0 ) {
									...
									$showimageComment = ''.PATH.'/teacher/images_/'.$INRresult_Comment['image'];
								} else {
									$showimageComment = "../assets/images/defalut.jpg";	
								}
             ...
             <div class="post-comment-container jzt_a_border" comment_id='<?php  echo $valueForComm['comment_id']  ?>'>
                <div class=" pull-left-child">
                    <div class="post-comment-avatar-post" style="background:url('<?php echo 
					$showimageComment; ?>');">
                    </div>
                </div>
...
<?php   
	...
	if($INRresult_post['delete_status'] == 0) {
...
<?php 
	...
	echo ' <div class="pull-right-comment" style="text-align:center; padding-bottom:40px; color:red;"> '.$INRresult_post['name'].'  no longer part of the system </div>';	
...
?>
    ...
    <div class="videocallsomeone" id="showTheCallers">
    <!-----
    	<div class="col-md-12 innrnewalert">
        	<img src="" width="100px" height="100px" class="col-md-5">
            <div class="col-md-7">
            	<h5> dsdsdsd</h5>
                <h6>dsdasdad</h6>
            </div>
     ...
     <form action="../video.php" method="post" class="hidden">
                        <input type="hidden" class="hidden" name="user_name" value="<?php echo substr($_SESSION['te'],3) ?>" id="my_user_name_for_you">
                         <input type="hidden" class="hidden" name="user_id" value="<?php echo substr($_SESSION['te'],3) ?>">
                           <input type="hidden" class="hidden" name="type" value=<?php echo 0; ?>>
                           
                           <input type="hidden" class="hidden" name="department" value="<?php   echo $result_view_base['department']; ?>" id="tosele56859656f" dp_id="<?php   echo $result_view_base['department']; ?>">
